...
class Model(object):
    ...
    def create_model(self, optimizer):
        ...
        midle = tf.layers.dense(inputs=features, units=100, kernel_regularizer=tf.contrib.layers.l2_regularizer(0.001))  # 0.001  #Linear layer without regularizer
        relu = tf.nn.relu(midle)
        logits = tf.layers.dense(inputs=relu, units=self.num_classes, kernel_regularizer=tf.contrib.layers.l2_regularizer(0.001))
        predictions = {
